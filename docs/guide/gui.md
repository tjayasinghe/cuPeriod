...
5. **Switch **Analysis** to *Pre-whitening*** and press **Run pre-whitening**. The
   ...
   lobe is likely an alias — and unchecking **data** hides the dense data curve so the
   residual and window traces can be read on their own. Watch the **A/Asp** column: a
   row marked ✱ has an amplitude entangled with a correlated neighbour. For a g-mode
   star, the **Period spacing** tab scans for a regular spacing and draws the échelle
   diagram.
